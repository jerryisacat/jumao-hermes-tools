...
3. 草案需要包含：
   ...
   - 当前技术栈
   - 推荐开发阶段拆分
   - 移动端策略：必须明确询问用户项目是否需要移动端适配，或是否应采用移动端优先；确认后写入 `AGENTS.md`
   - 代码治理规则
   - CommitLog 规则
...
5. 移动端策略不要由 Agent 自行假设。需要先向用户确认：
   - 这个项目是否需要移动端适配？
   - 如果需要，是“移动端优先”还是“桌面端优先但兼容移动端”？
   - 是否有明确目标：响应式 Web、PWA、原生 App、小程序、平板适配或仅桌面端？

   确认后，在 `AGENTS.md` 中写入移动端策略和设计/测试规则；如果用户确认不需要移动端，也要写明“当前不要求移动端适配”，避免后续 Agent 误判。
...
7. `AGENTS_CHANGELOGS.md` 不是传统 release changelog，而是 AI Agent 工作审计日志。它应记录每一轮 AI Agent 对代码、文档、配置或仓库结构所做的修改，以及做出这些修改的因果链。
...
9. 每次任务结束前，Agent 都必须检查以下文件是否需要更新：
   - `AGENTS.md`
   - `AGENTS_CHANGELOGS.md`
   - `CODEGUIDE.md`

10. 如果仓库中已有规范文件，不要覆盖它们。先读取并继承已有约定，再指出是否需要补充。

11. 如果现有文档之间存在冲突，请明确指出：
    - 哪些文件冲突
    - 冲突点是什么
    - 你建议以哪个文件为准
    - 为什么

12. 本轮先只输出讨论稿，不要修改文件。讨论稿确认后，我会再让你初始化 `AGENTS.md`、`AGENTS_CHANGELOGS.md` 和 `CODEGUIDE.md`。
```
